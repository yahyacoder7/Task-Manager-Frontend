  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
          ...
          <>
            ...
            {categoryBreakdown.length > 3 && (
              <TouchableOpacity style={styles.toggleBtn} onPress={() => setExpandedCategory(!expandedCategory)}>
                ...
                <Ionicons name={expandedCategory ? 'chevron-up' : 'chevron-down'} size={16} color={THEME.brand} />
              </TouchableOpacity>
            )}
          </>
        )}
        ...
        <>
          ...
          <View style={styles.chartCard}>
            <View style={styles.timeGrid}>
              {[
                { label: 'صباح', icon: 'partly-sunny-outline' as const },
                { label: 'ظهر', icon: 'sunny-outline' as const },
                { label: 'مساء', icon: 'cloudy-night-outline' as const },
                { label: 'ليل', icon: 'moon' as const },
              ].map((period, idx) => {
                const match = timeDist.find((t: any) => t.label?.includes(period.label));
                return (
                  <View key={idx} style={styles.timeChip}>
                    <Ionicons name={period.icon} size={20} color={THEME.secondaryText} />
        ...
        {/* Workplan Progress */}
        {workplanSummary.length > 0 && (
          <>
            <Text style={styles.sectionTitle}>تقدم خطط العمل</Text>
            {(expandedWorkplan ? workplanSummary : workplanSummary.slice(0, 2)).map((wp, idx) => (
              <View key={idx} style={styles.wpCard}>
                <View style={styles.wpHeader}>
                  <Text style={styles.wpPercent}>{wp.percent}%</Text>
                  <Text style={styles.wpName}>{wp.name}</Text>
                </View>
                <View style={styles.wpBarTrack}>
                  <View style={[styles.wpBarFill, { width: `${wp.percent}%` }]} />
                </View>
                <Text style={styles.wpCount}>{wp.completedTodos}/{wp.totalTodos} مهام</Text>
              </View>
            ))}
            {workplanSummary.length > 2 && (
              <TouchableOpacity style={styles.toggleBtn} onPress={() => setExpandedWorkplan(!expandedWorkplan)}>
                <Text style={styles.toggleBtnText}>{expandedWorkplan ? 'عرض أقل' : 'عرض المزيد'}</Text>
                <Ionicons name={expandedWorkplan ? 'chevron-up' : 'chevron-down'} size={16} color={THEME.brand} />
              </TouchableOpacity>
            )}
          </>
        )}
